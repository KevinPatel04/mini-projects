...
class MainWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle('Omnipresent NPNT Client')
        self.setLayout(QVBoxLayout(self))
        self.CustomButtonsContainer()
        self.temp_nums = []
        self.fin_nums = []
        # Connection Check

        # Registeration Check Here

    def CustomButtonsContainer(self):
        container = QWidget()
        container.setLayout(QGridLayout())

        self.result_field = QLineEdit()
        btn_result = QPushButton('Enter',clicked = self.func_result)
        btn_clear = QPushButton('Clear', clicked = self.clear_clac)
        ...
        btn_mult = QPushButton('x', clicked=lambda:self.func_press('*'))
